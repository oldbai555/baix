...
func NewTimeWheel(name string, interval int64, scales int, maxCap int) *TimeWheel {
	...
	log.Infof("Init timerWhell name = %s is Done!", tw.name)
	return tw
}
...
func (tw *TimeWheel) addTimer(tID uint32, t *Timer, forceNext bool) error {
	defer func() {
		if err := recover(); err != nil {
			log.Errorf("addTimer function err : %v", err)
		}
		return
	}()
...
func (tw *TimeWheel) AddTimeWheel(next *TimeWheel) {
	...
	log.Infof("Add timerWhell[%s]'s next [%s] is succ!", tw.name, next.name)
}
...
func (tw *TimeWheel) run() {
	for {
		...
		for tID, timer := range curTimers {
			...
			err := tw.addTimer(tID, timer, true)
			if err != nil {
...
func (tw *TimeWheel) Run() {
	...
	log.Infof("timerwheel name = %s is running...", tw.name)
}
