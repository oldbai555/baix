...
func TestNewTimerScheduler(t *testing.T) {
	...
	for i := 1; i < 2000; i++ {
		...
		if err != nil {
			log.Errorf("create timer error", tID, err)
			break
		}
	}
	...
	go func() {
		...
		for df := range delayFuncChan {
			asyncDoDelayFunc(context.TODO(), df)
		}
	}()
...
func TestCancelTimerScheduler(t *testing.T) {
	Scheduler := NewAutoExecScheduler()
	...
	log.Infof("timerID1=%d ,timerID2=%d\n", timerID1, timerID2)
